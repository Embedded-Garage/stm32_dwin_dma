...
/* USER CODE BEGIN PFP */
static void uart1_rx_process(void);
static void dwin_led_callback(uint16_t address, uint16_t value);
/* USER CODE END PFP */
...
int main(void)
{
  ...
  MX_USART1_UART_Init();
  /* USER CODE BEGIN 2 */
  if (false == dwin_comm_init(&huart1)) {
    Error_Handler();
  }
...
static void uart1_rx_process(void) {
  uint16_t bufPos = UART_RX_BUF_SIZE - __HAL_DMA_GET_COUNTER(huart1.hdmarx);
